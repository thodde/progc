        ...
        if (myActivity == PH_Server) {
            if (externalFD != 0) {
                if(checkSocketHasData(externalFD)) {
                    ...
                    //TOOD change this to finalize handshake?
                    ...
                    clientFDs[0] = newsockfd;
                }
            }
            if (clientFDs[0] != 0) {
                if(checkSocketHasData(clientFDs[0])) {
                    if(!processDownstreamData(clientFDs[0]))
                        close(clientFDs[0]);
                }
            }
        }
...
bool PhysicalLayer::processControlFrame(Frame *inFrame) {
    ...
    if (strcmp(inFrame->payload, "listen") >= 0) {
        ...
        printf("Received command to listen for chat client conections on port %i\n", listenPort);
